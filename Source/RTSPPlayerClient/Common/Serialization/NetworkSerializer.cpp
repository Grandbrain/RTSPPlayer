...
namespace Common::Serialization {
    ...
    NetworkSerializer::NetworkSerializer()
        : NetworkSerializer(MemorySerializer::ByteOrder::BigEndian) {
    }

    /// Constructs a network serializer.
    /// \details Constructs a network serializer with the specified byte order.
    /// \param[in]  byteOrder   Byte order.
    NetworkSerializer::NetworkSerializer(MemorySerializer::ByteOrder byteOrder)
    ...
    /// \return Byte order.
    MemorySerializer::ByteOrder NetworkSerializer::byteOrder() const {
        return byteOrder_;
    }
    ...
    void NetworkSerializer::serialize(const NetworkFrame& frame,
        ...
        while (index < frameSize) {
            ...
            serializer.setByteOrder(byteOrder_);
            ...
            serializer.seek(8);
            serializer << Utilities::Helpers::crc16(datagram);
    ...
    void NetworkSerializer::deserialize(const QByteArray& datagram) {
        ...
        if (datagramVersion != DATAGRAM_PROTOCOL_VERSION ||
            ...
            datagramCRC16 != Utilities::Helpers::crc16(datagram, {8, 9}))
            return;

        while (serializer.bytesAvailable() > CHUNK_NOTIFICATION_HEADER_SIZE) {
            quint8 chunkID;
            serializer >> chunkID;
